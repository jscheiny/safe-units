...
export type CompleteUnit = {
    [D in Dimension]: Exponent;
};
...
type NonZeroKeys<Unit extends MinimalUnit> = {
    [D in keyof Unit]: Unit[D] extends 0 ? never : D;
}[keyof Unit];
...
type FillZeroesImpl<Unit extends MinimalUnit> = Unit & {
    [Dim in MissingDimensions<Unit>]: 0;
};
